...
public class LogAnalyzer
{
     ...
     public void printAllHigherThanNum(int num) {
         ArrayList<LogEntry> statusGreaterThan = new ArrayList<LogEntry>();
         for(LogEntry le : records) {
     ...
     public int countUniqueIPsInRange(int low, int high) {
         ArrayList<String> uniqueIPsBetweenLowHigh = new ArrayList<String>();
         int IPCount = 0;
         for (LogEntry le : records){
             String address = le.getIpAddress();
             int statusCode = le.getStatusCode();
             if (!uniqueIPsBetweenLowHigh.contains(address)) {
                if(statusCode >= low && statusCode <= high) {
                 uniqueIPsBetweenLowHigh.add(address);
                 IPCount++;
                }
             } 
         ...
         // return uniqueIPsBetweenLowHigh.size();
         return IPCount;
     }
     ...
     public int mostNumberVisitsByIP(HashMap<String, Integer> ipVisits) {
         // SUMMARY: returns maximum number of visits to website by single IP address
         int maxVisits = 0;
         for (Integer visits : ipVisits.values()){
             if (visits > maxVisits) {
                 maxVisits = visits;
             }
         }
     ...
     public ArrayList<String> iPsMostVisits(HashMap<String, Integer> mostVisits) {
         ...
         for (String address : mostVisits.keySet()){
             if (mostVisits.get(address).equals(max)){
